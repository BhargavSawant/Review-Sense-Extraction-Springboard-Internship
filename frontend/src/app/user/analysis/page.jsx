...
import { Search, Bell, Sparkles, UploadCloud, Layers, Tag, Heart } from "lucide-react";
...
export default function AnalysisPage() {
  ...
  const getSentimentColor = (sentiment) => {
    const colors = {
      positive: { bg: "bg-green-50 dark:bg-green-900/30", text: "text-green-700 dark:text-green-400", border: "border-green-200 dark:border-green-800" },
      negative: { bg: "bg-red-50 dark:bg-red-900/30", text: "text-red-700 dark:text-red-400", border: "border-red-200 dark:border-red-800" },
      neutral: { bg: "bg-yellow-50 dark:bg-yellow-900/30", text: "text-yellow-700 dark:text-yellow-400", border: "border-yellow-200 dark:border-yellow-800" },
    };
  ...
  const getEmotionFromSentiment = (sentiment) => {
    const emotions = {
      positive: {
        ...
        description: "The review expresses satisfaction, happiness, and positive experiences.",
        color: "#4CD4A5",
        intensity: "High Intensity"
      },
      negative: {
        name: "DISAPPOINTMENT / FRUSTRATION",
        description: "The review indicates frustration, disappointment, or negative experiences.",
        color: "#E95252",
        intensity: "High Intensity"
      },
      neutral: {
  ...
  const aspectDoughnutData = results?.aspects && results.aspects.length > 0 ? {
    labels: results.aspects.slice(0, 5).map(a => a.aspect),
    datasets: [{
      data: results.aspects.slice(0, 5).map(() => 1),
      backgroundColor: results.aspects.slice(0, 5).map(a => 
        a.sentiment === 'positive' ? '#4CD4A5' : 
        a.sentiment === 'negative' ? '#E95252' : '#FCD34D'
      ),
      borderWidth: 0,
    }]
  } : null;

  const confidenceBarData = results?.aspects && results.aspects.length > 0 ? {
    ...
    <>
      ...
      <div className="min-h-screen bg-[#FAF8F5] dark:bg-slate-900 transition-colors">
        ...
        <header className="h-16 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30 backdrop-blur-sm bg-[#FAF8F5]/80 dark:bg-slate-900/80 border-b border-[#E6E2DD] dark:border-slate-700">
          <h1 className="text-lg md:text-xl font-bold text-[#2C2C2C] dark:text-white">Analysis Page</h1>
              ...
              {mode === "bulk" && (
                <div>
                  ...
                  >
                    ...
                    <p className="font-bold text-[#2C2C2C] dark:text-white">Click or Drop Files Here</p>
                  ...
                  {bulkLoading && (
                    <div className="mt-4 text-center">
                      ...
                      <p className="text-sm text-gray-600 dark:text-slate-400 mt-2">Processing bulk reviews...</p>
                    </div>
                  )}
                  {error && mode === "bulk" && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-[#E6E2DD] dark:border-slate-700 shadow-sm flex items-center gap-6 relative overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        backgroundColor: results.sentiment?.toLowerCase() === 'positive' ? '#4CD4A520' : 
                                        results.sentiment?.toLowerCase() === 'negative' ? '#E9525220' : '#FCD34D20'
                      }}
                    ></div>
                    <div 
                      className="w-16 h-16 rounded-full flex items-center justify-center"
                      style={{ 
                        backgroundColor: results.sentiment?.toLowerCase() === 'positive' ? '#4CD4A530' : 
                                        results.sentiment?.toLowerCase() === 'negative' ? '#E9525230' : '#FCD34D30'
                      }}
                    >
                      <svg 
                        className="w-8 h-8" 
                        fill={results.sentiment?.toLowerCase() === 'positive' ? '#4CD4A5' : 
                              results.sentiment?.toLowerCase() === 'negative' ? '#E95252' : '#FCD34D'} 
                        viewBox="0 0 24 24"
                      >
                    ...
                    <div>
                      ...
                      <p 
                        ...
                        style={{ 
                          color: results.sentiment?.toLowerCase() === 'positive' ? '#4CD4A5' : 
                                 results.sentiment?.toLowerCase() === 'negative' ? '#E95252' : '#FCD34D'
                        }}
                      >
                  ...
                  <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl p-6 border border-[#E6E2DD] dark:border-slate-700 shadow-sm flex flex-col justify-center">
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <span className={`text-2xl font-bold ${results.confidence >= 0.5 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    ...
                    <div className="w-full h-4 bg-[#FAF8F5] dark:bg-slate-700 rounded-full overflow-hidden">
                      <div
                        ...
                        className={`h-full transition-all duration-1000 ease-out ${results.confidence >= 0.5 ? 'bg-green-600' : 'bg-red-600'}`}
                        style={{ width: "0%" }}
                      />
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                      {results.aspects && results.aspects.length > 0 ? (
                          ...
                          return (
                            ...
                            >
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span className={`text-[10px] bg-white dark:bg-slate-800 border ${colors.border} ${colors.text} px-2 py-0.5 rounded-full capitalize font-medium`}>
                                  {aspect.sentiment}
                                </span>
                              </div>
                        ...
                        <div className="text-center py-8 text-gray-500 dark:text-slate-400">
                          <p className="text-sm">No aspects detected in this review.</p>
                          <p className="text-xs mt-1">Try analyzing a more detailed review.</p>
                        </div>
                      )}
                    </div>
                    {results.total_aspects_found > 0 && (
                      <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-600">
                        <p className="text-xs text-gray-600 dark:text-slate-400 text-center">
                          Found <span className="font-bold text-blue-600 dark:text-blue-400">{results.total_aspects_found}</span> total aspects
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-[#E6E2DD] dark:border-slate-700 shadow-sm">
                    ...
                    <div className="space-y-3">
                      {(() => {
                        const emotion = getEmotionFromSentiment(results.sentiment);
                        const emotionColor = 
                          results.sentiment === 'positive' ? '#4CD4A5' :
                          results.sentiment === 'negative' ? '#E95252' : '#FCD34D';
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="text-sm text-[#2C2C2C] dark:text-slate-300 mb-3">
                              "...{results.aspects?.[0]?.text_span || 'absolutely love the interface'}..."
                            </p>
                          </div>
                        );
                      })()}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-[#E6E2DD] dark:border-slate-700 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
                      <div className="text-sm text-blue-600 dark:text-blue-400 font-semibold">Total Reviews</div>
                      <div className="text-3xl font-bold text-blue-700 dark:text-blue-400">{bulkResults.total_reviews}</div>
                    </div>
                    <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl p-4">
                      <div className="text-sm text-green-600 dark:text-green-400 font-semibold">Positive</div>
                      <div className="text-3xl font-bold text-green-700 dark:text-green-400">{bulkResults.overall_percentage.positive}%</div>
                    </div>
                    <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4">
                      <div className="text-sm text-yellow-600 dark:text-yellow-400 font-semibold">Neutral</div>
                      <div className="text-3xl font-bold text-yellow-700 dark:text-yellow-400">{bulkResults.overall_percentage.neutral}%</div>
                    </div>
                    <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-4">
                      <div className="text-sm text-red-600 dark:text-red-400 font-semibold">Negative</div>
                      <div className="text-3xl font-bold text-red-700 dark:text-red-400">{bulkResults.overall_percentage.negative}%</div>
                    </div>
                  </div>

                  {/* Key Insights */}
                  <div className="mb-6">
                    <h3 className="text-lg font-bold text-[#2C2C2C] dark:text-white mb-3">Key Insights</h3>
                    <div className="space-y-2">
                      {bulkResults.key_insights?.map((insight, idx) => (
                  ...
                  {/* Aspects Summary */}
                  <div>
                    <h3 className="text-lg font-bold text-[#2C2C2C] dark:text-white mb-3">Aspect Analysis</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {Object.entries(bulkResults.aspects || {}).map(([aspect, data]) => (
                        <div key={aspect} className="bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl p-4">
                          <div className="font-bold text-[#2C2C2C] dark:text-white mb-2 uppercase text-sm">{aspect}</div>
                          <div className="flex justify-between text-xs mb-2">
                            <span className="text-green-600 dark:text-green-400">Pos: {data.percentages.positive}%</span>
                            <span className="text-yellow-600 dark:text-yellow-400">Neu: {data.percentages.neutral}%</span>
                            <span className="text-red-600 dark:text-red-400">Neg: {data.percentages.negative}%</span>
                          </div>
                          <div className="text-xs text-gray-600 dark:text-slate-400">
                            Mentioned in {data.percentage_mentioned}% of reviews ({data.mentions} times)
                          </div>
                        </div>
                      ))}
                    </div>
